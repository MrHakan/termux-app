...
The `versionName` in `build.gradle` files of Termux and its plugin apps must follow the [semantic version `2.0.0` spec](https://semver.org/spec/v2.0.0.html) in the format `major.minor.patch(-prerelease)(+buildmetadata)`. When bumping `versionName` in `build.gradle` files and when creating a tag for new releases on GitHub, make sure to include the patch number as well, like `v0.1.0` instead of just `v0.1`. The `build.gradle` files and the `release` workflow validate the version as well and the build will fail if `versionName` does not follow the spec. See [`docs/RELEASING.md`](docs/RELEASING.md) for how release APKs are built, signed and published.
